...
In this page, we will quickly work through the Julia code necessary to load a network dataset, assign weights to it, and identify and score missing links. To run the code, you'll need to install [Julia](https://julialang.org/install/).

I also highly recommend installing an interactive programming environment, to make the tool easier to use. I recommend installing:

 - [Quarto](https://quarto.org)
 - [Visual Studio Code](https://code.visualstudio.com/).
 
After installing Quarto and Visual Studio Code, within Visual Studio Code go to the extensions window (gear icon -> extensions) and install the [Julia](https://marketplace.visualstudio.com/items?itemName=julialang.language-julia) and [Quarto](https://marketplace.visualstudio.com/items?itemName=quarto.quarto) extensions for Visual Studio Code. More details about all of the functions from the MissingLinks package are available in the [API Documentation](@ref). Details about the algorithm itself are in [the original paper](https://files.indicatrix.org/publications/bhagat-conway-compiano-ivie-missing-links.pdf).

## Load packages

First, we need to install and load some packages. The Missing Links code is distributed as a Julia package, so we need to load MissingLinks. We also need to load the GeoDataFrames package for
spatial data manipulation, the Plots library for visualization, the Graphs library for working with graph data structures, and the StatsBase library for statistics. All of these packages are available from the Julia general registry, and can be installed [with the Pkg-mode add command](https://docs.julialang.org/en/v1/stdlib/Pkg/).

First start a Julia "REPL" (interactive session); in Visual Studio Code, press Ctrl-Shift-P (Cmd-Shift-P on Mac) and search for and select "Julia: Start REPL;" a pane will open at the bottom of the window where you can run Julia code.

Make sure your cursor is in the new pane, and press `]` to enter Pkg-mode. Then type

```
add GeoDataFrames Plots Graphs StatsBase GeoFormatTypes MissingLinks DataFrames
```

and press enter to install the packages.

You ran this code in the REPL. All of the other code you'll run should be in your notebook interface so you can save it. While it is optional, I do recommend [creating a Julia environment](https://pkgdocs.julialang.org/v1/environments/) to keep MissingLinks code separate from any other Julia projects you may work on.
...
If you're using Quarto and VSCode, create a new Quarto file (File -> New File, search for Quarto), and then place this code between triple-backticks to create a Julia "cell", then hover over it and click "run cell". Your quarto document will look like this:
