...
in_weight = in_weight + sum(downtown$weight)
```
...
in_weight = in_weight + sum(food$weight)
```
...
in_weight = in_weight + sum(hospitals$weight)
```
...
in_weight = in_weight + sum(major_emp$weight)
```
...
in_weight = in_weight + sum(park_ride$weight)
```
...
in_weight = in_weight + sum(schools$weight)
```
...
in_weight = in_weight + sum(sports_ent$weight)
```
...
in_weight = in_weight + sum(transit_center$weight)
```
...
in_weight = in_weight + sum(uni$weight)
```
...
in_weight = in_weight + sum(bus_stops$weight)
```
...
in_weight = in_weight + sum(lr_stops$weight)
```
...
in_weight = in_weight + sum(hotels$weight)
```
...
in_weight = in_weight + sum(libraries$weight)
```
...
in_weight = in_weight + sum(medical$weight)
```
...
in_weight = in_weight + sum(religious$weight)
```
...
in_weight = in_weight + sum(civic$weight)
```
...
in_weight = in_weight + sum(retail$weight)
```
...
parcel_weights = snap_pts %>%
    ...
    st_drop_geometry() %>%
    group_by(nearest) %>%
    summarize(weight=sum(weight), type=paste(sort(unique(type)), collapse=", "))

parcels$weight = 0
parcels$weight[parcel_weights$nearest] = parcel_weights$weight

parcels$type = NA_character_
parcels$type[parcel_weights$nearest] = parcel_weights$type
...
## Sanity check
```{r}
# Our weight for those that did snap plus those that are not snapped plus those that failed to snap should be our input weight
out_weight = sum(weighted_parcels$dest_weight) + sum(point_locations[!point_locations$snap_to_parcels,]$weight) + sum(snap_pts[snap_pts$nearest_dist >= units::as_units(100, "m"),]$weight)
stopifnot(all.equal(out_weight, in_weight))
```
