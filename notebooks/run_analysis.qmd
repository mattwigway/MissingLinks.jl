...
function flattento2d_chtype!(geom)
    ...
    return GC.@preserve new_geom ArchGDAL.IGeometry(GDAL.ogr_g_clone(new_geom.ptr))
end
```
...
```{julia}
# sanity checks, should be everything except parks and greenways
dest_points = GeoDataFrames.read(joinpath(DATAAXLE_PATH, "all_point_locations.gpkg"))

@chain dest_points begin
    groupby([:snap_to_parcels, :type, :weight])
    @combine(:n=length(:weight), :total=sum(:weight))
end
```
...
dest_points_unsnapped.geom = reproject(dest_points_unsnapped.geom, GFT.EPSG(4326), GFT.EPSG(32119), always_xy=true)
...
@assert sum(parks.weight) ≈ 497 # expected weight calculated on R side
...
# should be very close to 2 - confirmed 393 points that are affected by island removal, ~2%.
...
```{julia}
candidate_links = deduplicate_links(G, all_candidate_links, matrix, 100)
```

## Score the links

```{julia}
scores = score_links(x -> x < 1609 * 2, G, candidate_links, matrix, origin_weights, dest_weights, 1609 * 2)
```
...
```{julia}
@time scores_1mi = score_links(x -> x < 1609, G, candidate_links, matrix, origin_weights, dest_weights, 1609)
@time scores_3mi = score_links(x -> x < 1609 * 3, G, candidate_links, matrix, origin_weights, dest_weights, 1609 * 3)
```

### Gravity function

```{julia}
const β = open(x -> Base.read(x, Float64), joinpath(DATA_PATH, "beta.float64"))
```

```{julia}
decayfunc(x) = x > MAX_DIST_M ? 0.0 : exp(β*x)
@time scores_negexp = score_links(decayfunc, G, candidate_links, matrix, origin_weights, dest_weights, 5250)
```
...
## Store for service area analysis

```{julia}
result = (
    graph=G,
    links=candidate_links,
    scores_1mi=scores_1mi,
    scores_2mi=scores,
    scores_3mi=scores_3mi,
    scores_negexp=scores_negexp
)

serialize(joinpath(DATA_PATH, "data", "link_data.jlobj"), result)
```
