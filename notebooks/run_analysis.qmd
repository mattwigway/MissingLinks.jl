...
import Dates: now
import DrWatson: projectdir
CONFIG = TOML.parsefile(projectdir("config.toml"))
...
```{julia}
matrix = create_matrix(G, UInt16, origins=1:10000)
```
...
```{julia}
scores_1mi = score_links(x -> x < 1609, candidate_links, matrix, origin_weights, dest_weights, 1609)
scores_3mi = score_links(x -> x < 1609 * 3, candidate_links, matrix, origin_weights, dest_weights, 1609 * 3)
```
...
```{julia}
decayfunc(x) = x > 5250 ? 0.0 : exp(β*x)
scores_negexp = score_links(decayfunc, candidate_links, matrix, origin_weights, dest_weights, 5250)
```
