...
```{julia}
w = ones(nv(G))
scores = score_links(x -> x < 1609 * 2, candidate_links, matrix, w, w, 1609 * 2)
```
...
```{julia}
parcels = GeoDataFrames.read(joinpath(DATA_PATH, "parcels", "Parcel_LandUse.shp"));
...
CSV.write(joinpath(DATA_PATH, "parcels", "land_use_weights_template.csv"), df)
```

```{julia}
land_use_codes = CSV.read(joinpath(DATA_PATH, "parcels", "land_use_weights.csv"), DataFrame)
...
```{julia}
weights = create_graph_weights(G, parcels)
```